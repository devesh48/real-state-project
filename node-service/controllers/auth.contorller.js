...
export const google = async (req, res, next) => {
    try {
        const user = await Estate.findOne({email: req.body.email});
        if (user) {
            ...
            const newUser = new Estate({ username: req.body.name.split(" ").join("").toLowerCase() + Math.random().toString(36).slice(-4), email: req.body.email, password: hashedPassword, photo: req.body.photo});
